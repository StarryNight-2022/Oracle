...
class MFModel(torch.nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, model_id, prompt):
        model_id = torch.tensor(model_id, dtype=torch.long).to(self.get_device()) # [len(model_list), 1]

        model_embed = self.P(model_id) # [len(model_list), dim]
        ...
        prompt_embed = torch.tensor(prompt_embed, device=self.get_device()) # [embedding_dim]
        prompt_embed = self.text_proj(prompt_embed) # [dim]

        return self.classifier(model_embed * prompt_embed).squeeze() # [len(model_list), num_classes]
