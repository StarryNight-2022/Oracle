#NOTE: 这一版本将使用router/models/router/two_step_without_length_prediction.py

# 该文件负责实现对two_step_v1或是end2end.py生成的决策进行评价，
...
class Evaluator:
    def __init__(self, config:Dict, model_list:List[str], latency_constraint:float):
        ...
        self.iter_data:List = []
        self.latency_list:List[Dict[str, float]] = []
    ...
    def prepare_inputs(self):
        for idx in range(self.config["Data"]["num_data"]):
            ...
            for model in self.model_list:
                ...
                latency[model] = float(input[model]["runtime"])
    ...
    def __iter__(self):
        self.access_count += 1
        for item, latency in zip(self.iter_data, self.latency_list):
            yield (item[0], item[1], latency)
...
if __name__ == "__main__":
    ...
    from router.models.router.two_step_without_length_prediction import Router
    router = Router(config=config)
        
    model_list:List[str] = ["Qwen3-0.6B-temp-0-no-thinking", "Qwen3-14B-temp-0-no-thinking"]
    latency_constraint = 10.0  # seconds
    ...
    for idx, item in tqdm(enumerate(evaluator, start=1), total=len(evaluator)):
        prompt, oracle_choice, latency_dict = item
