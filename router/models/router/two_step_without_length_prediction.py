#NOTE: 该版本相较于two_step_v1.py与two_step_v2.py将不在使用输出长度预测，而是直接使用profile得到的latency实际值来进行step1的决策。
...
class Router():
    ...
    def step1(self, prompt:str, latency_dict:Dict[str, float], model_name_list:List[str], latency_constraint:float)->Tuple[torch.Tensor, List[str], bool]:
        '''
        @return: 
        ...
        if models_within == []:
            ...
            return [fastest_model], embedding_torch, True
        # 未超时，返回所有在约束内的模型
        else:
            return models_within, embedding_torch, False
    ...
    def route(self, prompt:str, latency_dict:Dict[str, float], model_name_list:List[str], latency_constraint:float)->str:
...
# That is an example of how to use the router
if __name__ == "__main__":
    ...
    latency_dict = {'Qwen3-0.6B-no-thinking': 2.5, 'Qwen3-14B-no-thinking': 5.0}
