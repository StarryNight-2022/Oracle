...
# 自定义内容
from router.models.modeling.modeling import MLP_6
...
def train_model(model, train_loader, val_loader, num_epochs=100, learning_rate=0.001, weight_decay=1e-5, device="cpu"):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs_prompt_embed, targets in train_loader:
            inputs_prompt_embed = inputs_prompt_embed.to(device)
            targets = targets.to(device)
            optimizer.zero_grad()
            outputs = model(inputs_prompt_embed)
        ...
        with torch.no_grad():
            for inputs_prompt_embed, targets in val_loader:
                inputs_prompt_embed = inputs_prompt_embed.to(device)
                targets = targets.to(device)
                outputs = model(inputs_prompt_embed)
                loss = criterion(outputs, targets)
...
def main(embedding_model:str):
    ...
    # 转换为TensorDataset
    train_dataset = TensorDataset(torch.tensor(X_train["input_embeddings_a"], dtype=torch.float32), torch.tensor(y_train, dtype=torch.float32))
    val_dataset = TensorDataset(torch.tensor(X_test["input_embeddings_a"], dtype=torch.float32), torch.tensor(y_test, dtype=torch.float32))
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    
    # 初始化模型
    model = MLP_6(embedding_dim=input_size, device=device, dtype=dtype)
    print(model)
