"""
CUDA_VISIBLE_DEVICES=0 nohup python Runtime/scripts/disable_thinking/qwen3-14B-temp-0-no-think.py > Runtime/logs/disable_thinking/qwen3-14B-temp-0-no-think.log 2>&1 &
"""
...
TEMPERATURE = 0
PARAMS = "14B"
THINKING_DIR = "no_think"  # or "think"
...
def initialize_model():
    ...
    """
    # 修改为你的实际模型路径
    model_path = f"/home/ouyk/project/Runtime/Model/Qwen3-{PARAMS}" #TODO
...
if __name__ == "__main__":
    ...
    outputs_dir = os.path.join(runtime_dir, "Oracle", f"outputs-qwen3-{PARAMS}")
    ...
    for idx, sample in enumerate(dataset, start=1):
        ...
        # question += "You only need to predict the length of your response. Print the estimated number of words in your response."
        question += "Before responding to the above instruction, you have to predict the length of your response. Print the estimated number of tokens in your response in the first line. Then change to a new line to respond to the instruction."
        ...
        out_path = os.path.join(outputs_dir, f"train_{idx}.jsonl")
        record["source_file"] = out_path

        with open(out_path, "w", encoding="utf-8") as jf:
            jf.write(json.dumps(record, ensure_ascii=False) + "\n")

        print(f"[{idx}/{total}] 已写入 JSONL: {out_path} | index={idx} | 耗时: {duration:.2f}s")
